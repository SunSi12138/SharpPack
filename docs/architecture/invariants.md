...
| Public API surface | Add CLR-public API only with explicit review and an intentional baseline update. | Removing, narrowing, or changing the signature of a shipped `SharpPack.Core` or `SharpPack.Streaming` API in 1.x, including public-hidden generated-code ABI. | `eng/baselines/public-api/SharpPack.Core.txt`, `eng/baselines/public-api/SharpPack.Streaming.txt`, and `tools/SharpPack.Baselines` verification. |
