...
[Preserve]
public sealed class BrotliFormatter : SharpPackFormatter<byte[]>
{
    ...
    [Preserve]
    public override void Serialize<TBufferWriter>(ref SharpPackWriter<TBufferWriter> writer, scoped ref byte[]? value)
    {
        if (value == null)
        {
    ...
    [Preserve]
    public override void Deserialize(ref SharpPackReader reader, scoped ref byte[]? value)
    {
        var uncompressedLength = reader.ReadUnmanaged<int>();
        if (uncompressedLength == SharpPackCode.NullCollection)
        {
            value = null;
            return;
        }
        ...
        if (uncompressedLength < 0)
        {
            SharpPackSerializationException.ThrowInvalidLength(uncompressedLength);
        }
        ...
        {
            SharpPackSerializationException.ThrowDecompressionSizeLimitExceeded(decompressionSizeLimit, uncompressedLength);
        }
...
[Preserve]
public sealed class BrotliFormatter<T> : SharpPackFormatter<T>
{
    internal const int DefaultDecompssionSizeLimit = 1024 * 1024 * 128; // 128MB
    ...
    public override void Serialize<TBufferWriter>(ref SharpPackWriter<TBufferWriter> writer, scoped ref T? value)
    {
        ...
        try
        {
            var coWriter = new SharpPackWriter<BrotliCompressor>(ref compressor, writer.OptionalState);
    ...
    public override void Deserialize(ref SharpPackReader reader, scoped ref T? value)
    {
        ...
        using var coReader = new SharpPackReader(decompressedSource, reader.OptionalState);
