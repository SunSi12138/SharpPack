...
internal static class Extensions
{
    ...
    public static bool IsUnmanagedRawCopyDisabled(
        this ITypeSymbol type,
        ReferenceSymbols references)
    {
        var visiting = new HashSet<ITypeSymbol>(
            SymbolEqualityComparer.Default);
        return IsUnmanagedRawCopyDisabledCore(type);

        bool IsUnmanagedRawCopyDisabledCore(ITypeSymbol candidate)
        {
            ...
            if (candidate.AllInterfaces.Any(item =>
                    SymbolEqualityComparer.Default.Equals(
                        item,
                        references.ISharpPackUnmanagedRawCopyDisabled)))
            {
                return true;
            }

            if (candidate is INamedTypeSymbol aggregate &&
                aggregate.IsGenericType &&
                IsFormatterAwareUnmanagedAggregate(aggregate))
            {
                foreach (var argument in aggregate.TypeArguments)
                {
                    if (IsUnmanagedRawCopyDisabledCore(argument))
                    {
                        return true;
                    }
                }
            ...
            if (candidate is not INamedTypeSymbol named ||
                !candidate.TryGetSharpPackableType(
                    references,
                    out var generateType,
                    out _) ||
                generateType is not GenerateType.Object ||
            ...
            try
            {
                foreach (var member in named.GetAllMembers())
                {
                    ...
                    var include = member.ContainsAttribute(
                        references.SharpPackIncludeAttribute);
                    var ignore = member.ContainsAttribute(
                        references.SharpPackIgnoreAttribute);
                    if (ignore ||
                        (!include &&
                    ...
                    if (member is IPropertySymbol
                        { GetMethod: null, SetMethod: not null } or
                        IPropertySymbol { IsIndexer: true })
                    {
                        continue;
                    }

                    if ((references.SharpPackCustomFormatterAttribute is { }
                            formatterAttribute &&
                         member.GetImplAttribute(formatterAttribute) is not null) ||
                        (references.SharpPackCustomFormatter2Attribute is { }
                            genericFormatterAttribute &&
                         member.GetImplAttribute(genericFormatterAttribute) is not null))
                    {
                        return true;
                    }
                    ...
                    if (memberType is not null &&
                        IsUnmanagedRawCopyDisabledCore(memberType))
                    {
                        return true;
                    }
                }
        ...
        bool IsFormatterAwareUnmanagedAggregate(INamedTypeSymbol type)
        {
            if (type.EqualsUnconstructedGenericType(
                    references.KnownTypes.System_Nullable_T) ||
                type.EqualsUnconstructedGenericType(
                    references.KnownTypes.System_Collections_Generic_KeyValuePair_T))
            {
                return true;
            }
    ...
    public static bool ContainsTypeParameter(this ITypeSymbol type)
    {
        ...
        if (type is INamedTypeSymbol named)
        {
            ...
            {
                if (argument.ContainsTypeParameter())
                {
                    return true;
                }
            }
    ...
    public static bool TryGetSharpPackableType(this ITypeSymbol symbol, ReferenceSymbols references, out GenerateType generateType, out SerializeLayout serializeLayout)
    {
        var memPackAttr = symbol.GetAttribute(references.SharpPackableAttribute);
        ...
        {
            var memPackUnionFormatterAttr = symbol.GetAttribute(references.SharpPackUnionFormatterAttribute);
        ...
        {
            // SharpPackable has three attribtues
    ...
    public static bool IsSharpPackableNoGenerate(this ITypeSymbol symbol, ReferenceSymbols references)
    {
        var memPackAttr = symbol.GetAttribute(references.SharpPackableAttribute);
        var packableCtorArgs = memPackAttr?.ConstructorArguments;
        if (memPackAttr == null || packableCtorArgs == null)
    ...
    public static bool IsWillImplementSharpPackUnion(this ITypeSymbol symbol, ReferenceSymbols references)
    {
        return symbol.IsAbstract && symbol.ContainsAttribute(references.SharpPackUnionAttribute);
    }
