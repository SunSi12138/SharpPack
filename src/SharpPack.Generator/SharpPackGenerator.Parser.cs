...
{
    SharpPackable, // ISharpPackable<> or [SharpPackable]
    Unmanaged,
    Nullable,
    UnmanagedNullable,
    KnownType,
    ...
    Blank, // blank marker
    CustomFormatter, // used [SharpPackCustomFormatterAttribtue]
}
...
public partial class TypeMeta
{
    ...
    public TypeMeta(INamedTypeSymbol symbol, ReferenceSymbols reference)
    {
        ...
        symbol.TryGetSharpPackableType(reference, out var generateType, out var serializeLayout);
        ...
        this.Members = symbol.GetAllMembers() // iterate includes parent type
            ...
            .Where(x =>
            {
                var include = x.ContainsAttribute(reference.SharpPackIncludeAttribute);
                var ignore = x.ContainsAttribute(reference.SharpPackIgnoreAttribute);
        ...
        this.IsUnion = symbol.ContainsAttribute(reference.SharpPackUnionAttribute);
        this.IsRecord = symbol.IsRecord;
        this.OnSerializing = CollectMethod(reference.SharpPackOnSerializingAttribute, IsValueType, isReader: false);
        this.OnSerialized = CollectMethod(reference.SharpPackOnSerializedAttribute, IsValueType, isReader: false);
        this.OnDeserializing = CollectMethod(reference.SharpPackOnDeserializingAttribute, IsValueType, isReader: true);
        this.OnDeserialized = CollectMethod(reference.SharpPackOnDeserializedAttribute, IsValueType, isReader: true);
    ...
    IMethodSymbol? ChooseConstructor(INamedTypeSymbol symbol, ReferenceSymbols reference)
    {
        ...
        var ctorWithAttrs = ctors.Where(x => x.ContainsAttribute(reference.SharpPackConstructorAttribute)).ToArray();

        if (Symbol.IsUnmanagedType)
        {
            if (ctorWithAttrs.Length != 0)
            {
                ctorInvalid = DiagnosticDescriptors.UnamangedStructSharpPackCtor;
            }
            return null;
        }
    ...
    public bool Validate(TypeDeclarationSyntax syntax, IGeneratorContext context, bool unionFormatter)
    {
        ...
        if (Symbol.BaseType != null)
        {
            ...
            foreach (var item in Symbol.GetAllMembers(withoutOverride: false))
            {
                if (item.IsOverride)
                {
                    var include = item.ContainsAttribute(reference.SharpPackIncludeAttribute);
                    var ignore = item.ContainsAttribute(reference.SharpPackIgnoreAttribute);
                    if (include || ignore)
                    {
                        var location = item.Locations.FirstOrDefault() ?? syntax.Identifier.GetLocation();

                        var attr = include ? "SharpPackInclude" : "SharpPackIgnore";
        ...
        // ALl Members
        if (Members.Length >= 250) // SharpPackCode.Reserved1
        {
        ...
        foreach (var item in Members)
        {
            ...
            if (item.Kind == MemberKind.NonSerializable)
            {
                ...
                {
                    context.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.MemberIsNotSharpPackable, item.GetLocation(syntax), Symbol.Name, item.Name, item.MemberType.FullyQualifiedToString()));
                    noError = false;
                }
            }
        ...
        {
            // All members must annotate SharpPackOrder
            foreach (var item in Members)
            {
            ...
            // Annotated SharpPackOrder must be continuous number from zero if GenerateType.Object.
    ...
    UNION_VALIDATIONS:
        if (IsUnion)
        {
            ...
            foreach (var item in UnionTags)
            {
                ...
                if (!item.Type.ContainsAttribute(reference.SharpPackableAttribute))
                {
                    context.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.UnionMemberMustBeSharpPackable, syntax.Identifier.GetLocation(), Symbol.Name, item.Type.Name));
                    noError = false;
                }
            }
        }
...
partial class MemberMeta
{
    ...
    public MemberMeta(ISymbol symbol, IMethodSymbol? constructor, ReferenceSymbols references, int sequentialOrder)
    {
        ...
        var orderAttr = symbol.GetAttribute(references.SharpPackOrderAttribute);
        if (orderAttr != null)
        {
        ...
        if (references.SharpPackCustomFormatterAttribute != null)
        {
            var genericFormatter = false;
            var customFormatterAttr = symbol.GetImplAttribute(references.SharpPackCustomFormatterAttribute);
            if (customFormatterAttr == null && references.SharpPackCustomFormatter2Attribute != null)
            {
                customFormatterAttr = symbol.GetImplAttribute(references.SharpPackCustomFormatter2Attribute);
            ...
            if (customFormatterAttr != null)
            {
                ...
                if (genericFormatter)
                {
                    formatterName = CustomFormatter.GetAllBaseTypes().First(x => x.EqualsUnconstructedGenericType(references.SharpPackCustomFormatter2Attribute!))
                        .TypeArguments[0].FullyQualifiedToString();
                }
                else
                {
                    formatterName = $"ISharpPackFormatter<{MemberType.FullyQualifiedToString()}>";
                }
    ...
    static MemberKind ParseMemberKind(ISymbol? memberSymbol, ITypeSymbol memberType, ReferenceSymbols references)
    {
        ...
        else if (memberType.AllInterfaces.Any(x => x.EqualsUnconstructedGenericType(references.ISharpPackable)))
        {
            return MemberKind.SharpPackable;
        }
        else if (memberType.TryGetSharpPackableType(references, out var generateType, out var serializeLayout))
        {
            switch (generateType)
            {
                ...
                case GenerateType.Collection:
                    return MemberKind.SharpPackableCollection;
                case GenerateType.Union:
                    return MemberKind.SharpPackableUnion;
                case GenerateType.NoGenerate:
                default:
                    return MemberKind.SharpPackableNoGenerate;
            }
        }
        else if (memberType.IsWillImplementSharpPackUnion(references))
        {
            return MemberKind.SharpPackUnion;
        }
        else if (memberType.TypeKind == TypeKind.Array)
        {
            if (memberType is IArrayTypeSymbol array)
            {
                if (array.IsSZArray)
                {
                    ...
                    {
                        if (elemType.TryGetSharpPackableType(references, out var elemGenerateType, out _) && elemGenerateType is GenerateType.Object or GenerateType.VersionTolerant or GenerateType.CircularReference)
                        {
        ...
        else
        {
            ...
            {
                if (memberSymbol.ContainsAttribute(references.SharpPackAllowSerializeAttribute))
                {
