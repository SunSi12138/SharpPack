...
public class UnionTest
{
    ...
    {
        {
            ...
            var bin1 = SharpPackSerializer.Serialize((IForExternalUnion)one);
            var bin2 = SharpPackSerializer.Serialize((IForExternalUnion)two);

            var one2 = SharpPackSerializer.Deserialize<IForExternalUnion>(bin1);
            var two2 = SharpPackSerializer.Deserialize<IForExternalUnion>(bin2);
        ...
        }
        {
            ...
            var bin1 = SharpPackSerializer.Serialize((IGenericsUnion<DateTime>)one);
            var bin2 = SharpPackSerializer.Serialize((IGenericsUnion<string>)two);

            bin1[0].Should().Be(0);
            bin2[0].Should().Be(11);

            var one2 = SharpPackSerializer.Deserialize<IGenericsUnion<DateTime>>(bin1);
            var two2 = SharpPackSerializer.Deserialize<IGenericsUnion<string>>(bin2);
    ...
    public void ClosedGenericExternalUnionUsesGeneratedDefaultFactory()
    {
        ...
        var decoded =
            SharpPackSerializer.Deserialize<IClosedGenericsUnion<string>>(
                payload);
    ...
    public void DynamicUnion_IsGenericContextOwnedAndWireCompatible()
    {
        ...
        var firstPayload = SharpPackSerializer.Serialize(first, context);
        var secondPayload = SharpPackSerializer.Serialize(second, context);
        ...
        SharpPackSerializer.Deserialize<IDynamicBase>(firstPayload, context)
            .Should().BeOfType<Gen1>().Which.MyProperty.Should().Be(5678);
        SharpPackSerializer.Deserialize<IDynamicBase>(secondPayload, context)
