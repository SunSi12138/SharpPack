...
public class WriterTest
{
    ...
    public void BufferManagementTest()
    {
        ...
        var state = SharpPackWriterOptionalStatePool.Rent();
        try
        {
            var writer = new SharpPackWriter<SpanControlWriter>(ref buffer, state);
    ...
    public void FormatterExceptionDoesNotCorruptWriterDepth()
    {
        ...
        writer.Flush();
        SharpPackSerializer.Deserialize<int>(buffer.WrittenSpan).Should().Be(42);
    }
    ...
    public void WriteObjectHeaderTest()
    {
...

        {
            ...
            buffer.WrittenSpan[0].Should().Be(SharpPackCode.NullObject);
            buffer.Clear();
            state.Reset();
        }
        for (var i = 0; i < 250; i++)
        ...
        for (byte i = SharpPackCode.Reserved1; i <= SharpPackCode.NullObject; i++)
        {
            if (i == 0) break;
            var state = SharpPackWriterOptionalStatePool.Rent();
            var writer = new SharpPackWriter<ArrayBufferWriter<byte>>(ref buffer, state);
    ...
    public void CheckForReusableLinkedArrayBufferWriter()
    {
        ...
        var bin = SharpPackSerializer.Serialize(buffer);
        var v2 = SharpPackSerializer.Deserialize<BufferTest>(bin);
    ...
    sealed class ThrowingIntFormatter : SharpPackFormatter<int>
    {
