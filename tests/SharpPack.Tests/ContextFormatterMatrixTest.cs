...
public class ContextFormatterMatrixTest
{
    ...
    void AssertEquivalent<T>(T value)
    {
        ...
        var defaultValue = SharpPackSerializer.Deserialize<T>(defaultPayload);
        var contextValue = SharpPackSerializer.Deserialize<T>(defaultPayload, context);
        SharpPackSerializer.Serialize(contextValue)
            .Should().Equal(SharpPackSerializer.Serialize(defaultValue));
