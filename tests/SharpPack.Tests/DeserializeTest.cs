...
public partial class DeserializeTest
{
    ...
    public async Task StreamTest()
    {
        ...
        var bytes = SharpPackSerializer.Serialize(expected);

        var stream = new RandomStream(bytes);

        var result = await SharpPackSerializer.DeserializeAsync<int[]>(stream);
        result.Should().Equal(expected);

        // large size
        expected = Enumerable.Range(1, 100000).ToArray();
        bytes = SharpPackSerializer.Serialize(expected);

        stream = new RandomStream(bytes);
        result = await SharpPackSerializer.DeserializeAsync<int[]>(stream);
        result.Should().Equal(expected);
    }
    ...
    public async Task LargeSegmentedStreamWorksWithDefaultAndContext()
    {
        ...
        var result = await SharpPackSerializer.DeserializeAsync<byte[]>(
            new RandomStream(bytes));
        result.Should().Equal(expected);

        var context = new SharpPackSerializerContext();
        result = await SharpPackSerializer.DeserializeAsync<byte[]>(
            new RandomStream(bytes),
            context);
        result.Should().Equal(expected);
    }
    ...
    {
        byte[] bytes = SharpPackSerializer.Serialize(value);
        ...
        T? result = SharpPackSerializer.Deserialize<T>(sequence);
